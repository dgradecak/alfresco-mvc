...
public class DefaultAlfrescoMvcServletContextConfig implements WebMvcConfigurer {
  ...
  public void extendMessageConverters(List<HttpMessageConverter<?>> converters) {
	  ...
	  converters.stream().filter(c -> c instanceof MappingJackson2HttpMessageConverter).forEach(c -> {
	      ...
	      ObjectMapper objectMapper = objectMapperBuilder.failOnEmptyBeans(false).failOnUnknownProperties(false).build();
	  ...
	  // this is from alfresco config in org.alfresco.rest.framework.jacksonextensions.JacksonHelper.afterPropertiesSet()
  }    
}
